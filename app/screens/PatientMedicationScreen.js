...
function PatientMedicationScreen(props) {
  ...
  const handleModalSubmitAdd = async (medData) => {
    ...
    tempData['administerTime'] = convertAdmTimeToMilitary(
      medData.administerTime,
    );
  ...
  const handleEditMedication = (medID) => {
    ...
    const unparsedMedData = originalUnparsedData.filter(
  ...
  const admStrToTime = (admStr) => {
    ...
    admArray = admArray.map((item) => new Date(convertTimeMilitary(item)));
    return admArray;
  };
  ...
  const handleModalSubmitEdit = async () => {
    ...
    tempFormData['administerTime'] = convertAdmTimeToMilitary(
  ...
  const formatAdmString = (timeString) => {
    ...
    admArray = admArray.map((item) => formatMilitaryToAMPM(item)).join(', ');
    return admArray;
  };
  ...
  const convertAdmTimeToMilitary = (admTimeArray) => {
    admTimeArray = admTimeArray.map((item) =>
      formatTimeHM24(new Date(item), false),
    );
