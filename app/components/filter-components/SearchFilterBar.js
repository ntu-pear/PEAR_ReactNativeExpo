...
function SearchFilterBar({
  ...
  const setFilteredList = ({
    ...
    if (!isEmptyObject(SORT_OPTIONS)) {
      filteredList = sortArray(
        filteredList,
        FIELD_MAPPING[
          isEmptyObject(tempSelSort)
    ...
    for (var filter in tempSelDropdownFilters) {
      if (tempSelDropdownFilters[filter]['label'] != 'All') {
        filteredList = getSubFilteredList(
          filteredList,
          filter,
          'label',
    ...
    for (var filter in tempSelDatetimeFilters) {
      filteredList = getDatetimeFilteredList(
        filteredList,
        filter,
  ...
  const getDatetimeFilteredList = (
    ...
    if (filterOptionDetails[filter]['isFilter']) {
      for (var i = 0; i < datetimeFilterTypes.length; i++) {
        ...
        if (
          datetimeFilterType in tempSelDatetimeFilters[filter] &&
          tempSelDatetimeFilters[filter][datetimeFilterType] != null
        ) {
          const datetimeType = filterOptionDetails[filter]['type'];
          let selectedDatetime = null;
          if (datetimeType == 'date' && datetimeFilterType == 'min') {
            selectedDatetime = new Date(
              tempSelDatetimeFilters[filter][datetimeFilterType].setHours(
                0,
                0,
                0,
              ),
            );
          } else if (datetimeType == 'date' && datetimeFilterType == 'max') {
            selectedDatetime = new Date(
              tempSelDatetimeFilters[filter][datetimeFilterType].setHours(
                23,
                59,
            ...
            selectedDatetime = setSecondsToZero(
              new Date(tempSelDatetimeFilters[filter][datetimeFilterType]),
            );
          }
          filteredList = filterByDatetime(
            filteredList,
            filter,
            datetimeFilterType,
            selectedDatetime,
      ...
      filterOptionDetails[filter]['nestedFilter'].length > 0
    ) {
      ...
      filteredList.forEach((item) => {
        ...
        item[key].forEach((itemObj) => {
          ...
          const datetimeType = filterOptionDetails[filter]['type'];
          if (
            'min' in tempSelDatetimeFilters[filter] &&
            tempSelDatetimeFilters[filter]['min'] != null
          ) {
            let selectedDatetime = null;
            if (datetimeType == 'date') {
              selectedDatetime = new Date(
                tempSelDatetimeFilters[filter]['min'].setHours(0, 0, 0),
              );
            } else if (datetimeType == 'time') {
              selectedDatetime = setSecondsToZero(
                new Date(tempSelDatetimeFilters[filter]['min']),
              );
            }
          ...
          if (
            'max' in tempSelDatetimeFilters[filter] &&
            tempSelDatetimeFilters[filter]['max'] != null
          ) {
            let selectedDatetime = null;
            if (datetimeType == 'date') {
              selectedDatetime = new Date(
                tempSelDatetimeFilters[filter]['max'].setHours(0, 0, 0),
              );
            } else if (datetimeType == 'time') {
              selectedDatetime = setSecondsToZero(
                new Date(tempSelDatetimeFilters[filter]['max']),
              );
            }
            if (new Date(itemObj[fieldKey]) > selectedDatetime) {
              filterPass = false;
  ...
  const filterByDatetime = (
    ...
    if (datetimeFilterType == 'min') {
      filteredList =
        filteredList.filter(
          (obj) => new Date(obj[fieldKey]) >= selectedDatetime,
        ) || [];
  ...
  const getSubFilteredList = (filteredList, filter, id, tempSelFilters) => {
    if (filterOptionDetails[filter]['isFilter']) {
      ...
      filteredList = getNestedSubFilteredList(
        filteredList,
        filter,
        id,
        tempSelFilters,
  ...
  const getNestedSubFilteredList = (
    filteredList,
    filter,
    id,
    tempSelFilters,
    ...
    filteredList.forEach((item) => {
      ...
      item[key].forEach((itemObj) => {
        ...
        if (isEmptyObject(filterOptionDetails[filter]['options'])) {
          ...
          if (
            itemObj[fieldKey] ==
            filterOptionDetails[filter]['options'][tempSelFilters[filter][id]]
          ) {
            keyItems.push({ ...itemObj });
          }
        }
  ...
  const searchBarComponent = () => {
    return (
      <SearchBar
        ...
        placeholder={
          SEARCH_OPTIONS.length > 1 ? 'Search' : `Search by ${searchOption}`
        }
      />
    );
  };
  ...
  const tabBarComponent = () => {
    return !isEmptyObject(VIEW_MODES) ? (
      <>
        <TabBar
  ...
  return (
    <Center testID={testID} backgroundColor={colors.white} zindex={1}>
      {tabBarComponent()}
      {hideSearchBar ? (
        <View
          flexDirection="row"
