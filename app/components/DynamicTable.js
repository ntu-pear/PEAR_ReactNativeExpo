...
function DynamicTable({
  ...
  const getHeaderData = () => {
    let tempHeaderData = [
      ...headerData.filter((x) => x != 'ID'),
      onClickEdit ? 'Edit' : [],
      onClickDelete ? 'Delete' : [],
    ];
    customColumns.forEach((item) => tempHeaderData.push(item.title));
    ...
    <ScrollView style={styles.scrollViewVertical}>
      <ScrollView horizontal>
        <View>
          <View>
            <Table
              borderStyle={{ borderWidth: 1, borderColor: colors.primary_gray }}
            >
              <Row
                style={styles.head}
